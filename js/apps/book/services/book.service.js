...
function query(filterBy) {
    if(filterBy) {
     ...
     price = price ?price: 0
        const booksToShow = gBooks.filter(book => book.title.includes(name) && book.listPrice.amount > price)
...
function _addBook(bookToEdit){
  var book=_creatBook(bookToEdit.name,bookToEdit.price)
  gBooks.unshift(book)
  _saveBooksToStorage()
